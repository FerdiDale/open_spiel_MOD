...
def main(_):
  ...
  for cur_episode in range(training_episodes): # qui si valutano contro bot random
    if cur_episode % int(1e3) == 0:
      win_rates = eval_against_random_bots(env, agents, random_agents, 100)
      logging.info("Starting episode %s, win_rates %s", cur_episode, win_rates)
      y_p1.append(win_rates[0])
      x_p1.append(cur_episode)
      y_p2.append(win_rates[1])
      x_p2.append(cur_episode)
    time_step = env.reset()
    while not time_step.last(): # qui si allenano tra loro gli agenti
      player_id = time_step.observations["current_player"]
  ...
  plt.plot(x_p1, y_p1, label = "giocatore 1")
  plt.plot(x_p2, y_p2, label = "giocatore 2")
  plt.legend(loc='best')
  plt.show()
